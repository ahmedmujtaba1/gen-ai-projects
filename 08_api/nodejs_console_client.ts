...
async function login(username: string, password: string): Promise<string> {
    try {
        const response = await axios.post(`${BASE_URL}/token`, `username=${username}&password=${password}`, {
            headers: {
...
async function updateTodo(token: string, todoId: number, title: string, description: string) {
    try {
        const response = await axios.put(`${BASE_URL}/todos/${todoId}`, { title, description }, {
            headers: { Authorization: `Bearer ${token}` }
        });
...
async function deleteTodo(token: string, todoId: number) {
    try {
